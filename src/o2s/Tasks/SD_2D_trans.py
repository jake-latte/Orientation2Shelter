import torch
import sys
import o2s
import o2s.Templates.vars_2D as template_2D

class SD_2D_trans(template_2D.Vars2D):
    task_name = "SD-2D_trans"
    input_map = {
        'sin_hd': 0,
        'cos_hd': 1,
        'x': 2,
        'y': 3,
        'sx': 4,
        'sy': 5
    }
    target_map = {
        'sin_sd': 0,
        'cos_sd': 1
    }
    default_params = template_2D.default_params
    get_vars = staticmethod(template_2D.get_vars)
    test_func = o2s.test.test_tuning
    test_func_args = dict(tuning_vars_list=['ego_SD', 'allo_SD', 'AV', 'x', 'y'])
    @staticmethod
    def create_data(config, vars, inputs, targets, mask):
        batch_size, n_timesteps = config.batch_size, config.n_timesteps
    
        inputs[:,:,SD_2D_trans.input_map['sin_hd']] = torch.sin(vars['hd'])
        inputs[:,:,SD_2D_trans.input_map['cos_hd']] = torch.cos(vars['hd'])
        inputs[:,:,SD_2D_trans.input_map['x']] = vars['x']
        inputs[:,:,SD_2D_trans.input_map['y']] = vars['y']
        inputs[:,:,SD_2D_trans.input_map['sx']] = vars['sx'].reshape((batch_size,1)).repeat((1,n_timesteps))
        inputs[:,:,SD_2D_trans.input_map['sy']] = vars['sy'].reshape((batch_size,1)).repeat((1,n_timesteps))
    
        targets[:,:,SD_2D_trans.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,SD_2D_trans.target_map['cos_sd']] = torch.cos(vars['sd'])
    
        mask[:,:config.init_duration] = False
    
        return inputs, targets, mask
    def __init__(self, **kwargs):
        self.get_vars_func = self.get_vars
        self.get_subtask_vars_funcs = {}
        super().__init__(
            name=self.task_name,
            task_specific_params=self.default_params,
            get_vars_func=self.get_vars_func,
            create_data_func=self.create_data,
            input_map=self.input_map,
            target_map=self.target_map,
            test_func=self.test_func,
            test_func_args=self.test_func_args,
            **kwargs
        )

# input_map = {
#     'sin_hd': 0,
#     'cos_hd': 1,
#     'x': 2,
#     'y': 3,
#     'sx': 4,
#     'sy': 5
# }
#
# target_map = {
#     'sin_sd': 0,
#     'cos_sd': 1
# }
#
#
#
# def create_data(config, vars, inputs, targets, mask):
#     batch_size, n_timesteps = config.batch_size, config.n_timesteps
#
#     inputs[:,:,input_map['sin_hd']] = torch.sin(vars['hd'])
#     inputs[:,:,input_map['cos_hd']] = torch.cos(vars['hd'])
#     inputs[:,:,input_map['x']] = vars['x']
#     inputs[:,:,input_map['y']] = vars['y']
#     inputs[:,:,input_map['sx']] = vars['sx'].reshape((batch_size,1)).repeat((1,n_timesteps))
#     inputs[:,:,input_map['sy']] = vars['sy'].reshape((batch_size,1)).repeat((1,n_timesteps))
#
#     targets[:,:,target_map['sin_sd']] = torch.sin(vars['sd'])
#     targets[:,:,target_map['cos_sd']] = torch.cos(vars['sd'])
#
#     mask[:,:config.init_duration] = False
#
#     return inputs, targets, mask
#
#
#
# SD_2D_TRANS_TASK = o2s.task.Task('SD-2D_trans',
#                     task_specific_params=template_2D.default_params, 
#                     get_vars_func=template_2D.get_vars,
#                     create_data_func=create_data,
#                     input_map=input_map,
#                     target_map=target_map,
#                     test_func=o2s.test.test_tuning,
#                     test_func_args=dict(tuning_vars_list=['ego_SD', 'allo_SD', 'AV', 'x', 'y']))
#
#
