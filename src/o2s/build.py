...
def build(task: o2s.task.Task, net: o2s.net.RNN = None, optimiser: torch.optim.Optimizer = None, train_losses: List[float] = None, test_losses: List[float] = None, use_wandb: bool = False):
    config = task.config
    config.seed()
    ...
    wandb_run = None
    if use_wandb:
        wandb_config = dict(config.dict)
        if isinstance(wandb_config.get('device'), torch.device):
            wandb_config['device'] = str(wandb_config['device'])
        wandb_run = wandb.init(
            entity='jakelaherty-gatsby-computational-neuroscience-unit',
            project='orientation-2-shelter',
            config=wandb_config,
            name=f'{config.time}-{config_name}',
            dir=build_dir,
            reinit=True,
        )
    ...
    dataset = o2s.data.TaskDataset(task, include_noise=(not task.config.conserve_vram))
    train_batch_loader = DataLoader(dataset, batch_size=config.batch_size, num_workers=config.num_loader_workers, collate_fn=dataset.collate_fn)

    test_batch = dataset.get_batch()
    ...
    for epoch, epoch_losses in o2s.train.train(task, net, train_batch_loader, optimiser, n_epochs=config.n_epochs, killtime=killtime):
        epoch += epoch_offset
        ...
        if use_wandb and wandb_run is not None:
            epoch_train_loss = float(np.mean(epoch_losses)) if len(epoch_losses) > 0 else None
            stats = {
    ...
    # Free memory
    del net, optimiser
    del dataset, train_batch_loader, test_batch
    del train_losses, test_losses
...
def build_from_command_line():
    ...
    if ':' in args[1]:
        task_name = args[1].split(':')[0]
        subtask_name = args[1].split(':')[-1]
    else:
        task_name = args[1]
        subtask_name = None
    ...
    task = None
    try:
        task = o2s.task.Task.named(task_name)
        if subtask_name is not None:
            assert subtask_name in task.get_subtask_vars_funcs.keys()
    except:
    ...
    if len(args) > 1:
        ...
        for arg_i in range(1, len(args)):
            ...
            # If contunuing a previous build:
                ...
                if task.config.optimiser_name == 'Adam':
                    ...
                    optimiser = o2s.hessianfree.HessianFree(
                        net.parameters(), lr=task.config.max_lr,
        ...
        if '-P' in args:
            ...
            for i in range(num_models):
                ...
                p = mp.Process(target=build, args=(parallel_task,), kwargs={'use_wandb': use_wandb})
                p.start()
                processes.append(p)
            ...
            if subtask_name is not None:
                task = task.get_subtask(subtask_name)
                if '-c' not in args:
                    task.config.make_name(include=name_param_keys)
