...
class SD_0D_EC(template_0D.Vars0D, template_EC.VarsEC):
    task_name = "SD-0D_EC"
    input_map = {
        'av': 0
    }
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        inputs[:,:,SD_0D_EC.input_map['av']] = vars['av']
    
        targets[:,:,SD_0D_EC.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,SD_0D_EC.target_map['cos_sd']] = torch.cos(vars['sd'])
...
#
#
# SD_0D_EC_TASK = o2s.task.Task('SD-0D_EC',
