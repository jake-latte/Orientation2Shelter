...
class HD_SD_2D_curriculum(template_0D.Vars0D, template_1D.Vars1DVecvel, template_2D.Vars2DVecvel):
    task_name = "HD_SD-2D_curriculum"
    default_params = {
        'stage': 1e-9,
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        targets[:,:,HD_SD_2D_curriculum.target_map['sin_hd']] = torch.sin(vars['hd'])
        targets[:,:,HD_SD_2D_curriculum.target_map['cos_hd']] = torch.cos(vars['hd'])
        targets[:,:,HD_SD_2D_curriculum.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,HD_SD_2D_curriculum.target_map['cos_sd']] = torch.cos(vars['sd'])
...
#
#
# HD_SD_2D_CURRICULUM_TASK = o2s.task.Task('HD_SD-2D_curriculum',
