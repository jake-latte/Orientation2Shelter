        ...
        for file in files:
            ...
            for i in order:
                ...
                session_label = torch.tensor(
                    self.animal_num_classes * animal_idx_array[i] + exp_idx_array[i], dtype=torch.long
                )

                session_oh = F.one_hot(
                    session_label,
                    num_classes=self.animal_num_classes * self.session_num_classes,
                ).to(inputs.dtype)
...
def preds_from_embeds(embeds, targets, n_outputs, l2_lambda):
    ...
    Y0 = torch.nan_to_num(targets, nan=0.0).to(targets.dtype)                  # (T, N)
...
def main(**kwargs):
    ...
    for k in range(cv_folds):
        ...
        model = FoundationModel(
            n_inputs = 7 * (6 + past_embed + future_embed),
        ...
        for epoch, train_losses, test_losses in iter_epochs(model, optim, train_loader, test_loader,
            ...
            lazy_metric = get_lazy_metric(model, init_params, test_loader)
