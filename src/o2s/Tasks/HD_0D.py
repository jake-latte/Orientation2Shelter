...
class HD_0D(template_0D.Vars0D):
    task_name = "HD-0D"
    target_map = {
        'sin_hd': 0,
        'cos_hd': 1
    ...
    test_func_args = dict(tuning_vars_list=['HD', 'AV'])
    @staticmethod
    def create_data(config, vars, inputs, targets, mask):
        inputs, mask = template_0D.fill_inputs(config, vars, inputs, mask)
    
        targets[:,:,HD_0D.target_map['sin_hd']] = torch.sin(vars['hd'])
        targets[:,:,HD_0D.target_map['cos_hd']] = torch.cos(vars['hd'])
...
#
#
# HD_0D_TASK = o2s.task.Task('HD-0D',
