...
class SD_0D_EC_highrank(template_0D.Vars0D, template_EC.VarsEC):
    task_name = "SD-0D_EC_highrank"
    input_map = {
        'av': 0
    }
    target_map = {
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        inputs[:,:,SD_0D_EC_highrank.input_map['av']] = vars['av']
    
        targets[:,:,SD_0D_EC_highrank.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,SD_0D_EC_highrank.target_map['cos_sd']] = torch.cos(vars['sd'])
