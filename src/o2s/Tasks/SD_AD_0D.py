...
class SD_AD_0D(template_0D.Vars0D):
    task_name = "SD_AD-0D"
    target_map = {
        'sin_sd': 0,
        'cos_sd': 1,
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        targets[:,:,SD_AD_0D.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,SD_AD_0D.target_map['cos_sd']] = torch.cos(vars['sd'])
        targets[:,:,SD_AD_0D.target_map['sin_ad']] = vars['sy'].reshape((-1,1)).repeat(1,config.n_timesteps)
        targets[:,:,SD_AD_0D.target_map['cos_ad']] = vars['sx'].reshape((-1,1)).repeat(1,config.n_timesteps)
...
#
#
# SD_AD_0D_TASK = o2s.task.Task('SD_AD-0D',
