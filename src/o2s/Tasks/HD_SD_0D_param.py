...
class HD_SD_0D_param(template_0D.Vars0D):
    task_name = "HD_SD-0D-param"
    default_params = {
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        inputs[:,:,HD_SD_0D_param.input_map['output_angle_is_hd']] = output_is_hd.int()
    
    
        targets[:,:,HD_SD_0D_param.target_map['sin_angle']][output_is_hd] = torch.sin(vars['hd'])[output_is_hd]
        targets[:,:,HD_SD_0D_param.target_map['cos_angle']][output_is_hd] = torch.cos(vars['hd'])[output_is_hd]
    
        ad = torch.remainder(vars['hd'] + vars['sd'], 2*np.pi)
        targets[:,:,HD_SD_0D_param.target_map['sin_angle']][~output_is_hd] = torch.sin(vars['sd'])[~output_is_hd]
        targets[:,:,HD_SD_0D_param.target_map['cos_angle']][~output_is_hd] = torch.cos(vars['sd'])[~output_is_hd]
    ...
    def __init__(self, **kwargs):
        ...
        self.get_subtask_vars_funcs = {
            ...
            "joint_unswapped": self.get_joint_unswapped_vars,
