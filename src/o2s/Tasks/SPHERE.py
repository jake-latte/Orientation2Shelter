...
class SPHERE(o2s.task.Task):
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        inputs[:,:init_duration,SPHERE.input_map['sin_theta']] = torch.sin(theta_vals).reshape((batch_size,1)).repeat((1,init_duration))
        inputs[:,:init_duration,SPHERE.input_map['cos_theta']] = torch.cos(theta_vals).reshape((batch_size,1)).repeat((1,init_duration))
        inputs[:,:init_duration,SPHERE.input_map['sin_phi']] = torch.sin(phi_vals).reshape((batch_size,1)).repeat((1,init_duration))
        inputs[:,:init_duration,SPHERE.input_map['cos_phi']] = torch.cos(phi_vals).reshape((batch_size,1)).repeat((1,init_duration))
    
        mask[:,:init_duration] = False
    
        targets[:,:,SPHERE.target_map['theta']] = theta_vals.reshape((batch_size,1)).repeat((1,n_timesteps))
        targets[:,:,SPHERE.target_map['phi']] = phi_vals.reshape((batch_size,1)).repeat((1,n_timesteps))
