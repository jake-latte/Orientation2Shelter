...
class HD_AD_0D(template_0D.Vars0D):
    task_name = "HD_AD-0D"
    target_map = {
        'sin_hd': 0,
        'cos_hd': 1,
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        targets[:,:,HD_AD_0D.target_map['sin_hd']] = torch.sin(vars['hd'])
        targets[:,:,HD_AD_0D.target_map['cos_hd']] = torch.cos(vars['hd'])
        targets[:,:,HD_AD_0D.target_map['sin_ad']] = vars['sy'].reshape((-1,1)).repeat(1,config.n_timesteps)
        targets[:,:,HD_AD_0D.target_map['cos_ad']] = vars['sx'].reshape((-1,1)).repeat(1,config.n_timesteps)
