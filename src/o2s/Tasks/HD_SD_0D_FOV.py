...
import o2s.Templates.vars_FOV as template_FOV

class HD_SD_0D_FOV(template_0D.Vars0D, template_FOV.VarsFOV):
    task_name = "HD_SD-0D_FOV"
    target_map = {
        'sin_hd': 0,
        'cos_hd': 1,
        'sin_sd': 2,
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        if 'shelter_in_FOV' not in vars:
            vars = template_FOV.add_fov_var(config, vars)
        inputs[:,:,template_0D.input_map['sx']][vars['shelter_in_FOV']] = vars['sx'].reshape((-1,1)).repeat((1,config.n_timesteps))[vars['shelter_in_FOV']]
        inputs[:,:,template_0D.input_map['sy']][vars['shelter_in_FOV']] = vars['sy'].reshape((-1,1)).repeat((1,config.n_timesteps))[vars['shelter_in_FOV']]
    
        targets[:,:,HD_SD_0D_FOV.target_map['sin_hd']] = torch.sin(vars['hd'])
        targets[:,:,HD_SD_0D_FOV.target_map['cos_hd']] = torch.cos(vars['hd'])
        targets[:,:,HD_SD_0D_FOV.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,HD_SD_0D_FOV.target_map['cos_sd']] = torch.cos(vars['sd'])
...
#
#
# HD_SD_0D_FOV_TASK = o2s.task.Task('HD_SD-0D_FOV',
