...
def get_vars(config):
    ...
    for t in range(init_duration, n_timesteps):
        ...
        max_k_x = torch.where(
            xv_step > 0,
            (max_xy - pos[:,t,0]) / xv_step,  # Positive direction
            (pos[:,t,0] - min_xy) / -xv_step  # Negative direction
        )

        max_k_y = torch.where(
            yv_step > 0,
            (max_xy - pos[:,t,1]) / yv_step,  # Positive direction
            (pos[:,t,1] - min_xy) / -yv_step  # Negative direction
        )
...
class Vars2DLinvel(o2s.task.Task):
