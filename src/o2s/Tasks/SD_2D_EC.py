...
class SD_2D_EC(template_2D.Vars2D, template_EC.VarsEC):
    task_name = "SD-2D_EC"
    target_map = {
        'sin_sd': 0,
        'cos_sd': 1
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        targets[:,:,SD_2D_EC.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,SD_2D_EC.target_map['cos_sd']] = torch.cos(vars['sd'])
