...
def create_data(config, inputs, targets, mask):
    ...
    vars = Tasks.vars_2D_vecvel.create_data(config, for_training=(inputs.shape[0] == config.batch_size and inputs.shape[1] == config.n_timesteps))
    inputs, mask = Tasks.vars_2D_vecvel.fill_inputs(config, inputs, mask, vars)
...
PI_HD_SD_2D_vecvel_TASK = Task('PI_HD_SD-2D_vecvel', 
                            ...
                            task_specific_params=Tasks.vars_2D_vecvel.default_params, 
                            create_data_func=create_data,
                            input_map=Tasks.vars_2D_vecvel.input_map,
