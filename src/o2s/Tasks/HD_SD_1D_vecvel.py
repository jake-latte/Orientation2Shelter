...
class HD_SD_1D_vecvel(template_1D_vecvel.Vars1DVecvel):
    task_name = "HD_SD-1D_vecvel"
    target_map = {
        'sin_hd': 0,
        'cos_hd': 1,
        'sin_sd': 2,
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        targets[:,:,HD_SD_1D_vecvel.target_map['sin_hd']] = torch.sin(vars['hd'])
        targets[:,:,HD_SD_1D_vecvel.target_map['cos_hd']] = torch.cos(vars['hd'])
        targets[:,:,HD_SD_1D_vecvel.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,HD_SD_1D_vecvel.target_map['cos_sd']] = torch.cos(vars['sd'])
