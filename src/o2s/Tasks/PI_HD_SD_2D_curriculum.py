...
class PI_HD_SD_2D_curriculum(template_0D.Vars0D, template_1D.Vars1DVecvel, template_2D.Vars2DVecvel):
    task_name = "PI_HD_SD-2D_curriculum"
    default_params = {
        'stage': 1e-9,
    ...
    def create_data(config, vars, inputs, targets, mask):
        ...
        targets[:,:,PI_HD_SD_2D_curriculum.target_map['x']] = vars['x']
        targets[:,:,PI_HD_SD_2D_curriculum.target_map['y']] = vars['y']
        targets[:,:,PI_HD_SD_2D_curriculum.target_map['sin_hd']] = torch.sin(vars['hd'])
        targets[:,:,PI_HD_SD_2D_curriculum.target_map['cos_hd']] = torch.cos(vars['hd'])
        targets[:,:,PI_HD_SD_2D_curriculum.target_map['sin_sd']] = torch.sin(vars['sd'])
        targets[:,:,PI_HD_SD_2D_curriculum.target_map['cos_sd']] = torch.cos(vars['sd'])
...
#
#
# PI_HD_SD_2D_CURRICULUM_TASK = o2s.task.Task('PI_HD_SD-2D_curriculum',
